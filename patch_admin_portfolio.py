...
old_code = """                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        {localPortfolioItems
                          ...
                          .map(({ item, index }) => (
                          <div key={`portfolio-item-${item.id || 'new'}-${index}`} className="relative group rounded-sm overflow-hidden border border-neutral-900 bg-[var(--color-bg)] aspect-square">
                            <img 
                              src={item.url} 
...
new_code = """                      {portfolioTab !== 'All' && (
                        <div className="text-[10px] text-neutral-500 font-sans italic mb-4">
                          Note: Drag-and-drop sorting is only available in the 'All' tab.
                        </div>
                      )}
                      {portfolioTab === 'All' ? (
                        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={(e) => handleDragEnd(e, 'portfolio', localPortfolioItems, setLocalPortfolioItems)}>
                          <SortableContext items={localPortfolioItems.map(i => i.id || '')} strategy={rectSortingStrategy}>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                          {localPortfolioItems
                            .filter(item => item.category === portfolioTab)
                            .map((item, index) => (
                            <div key={`portfolio-item-${item.id || 'new'}-${index}`} className="relative group rounded-sm overflow-hidden border border-neutral-900 bg-[var(--color-bg)] aspect-square">
                              <img 
                                src={item.url} 
...
with open('src/components/AdminPanel.tsx', 'w') as f:
    f.write(content)
print("Replaced portfolio map with Sortable")
