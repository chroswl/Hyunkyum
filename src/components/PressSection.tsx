    ...
    return (
      <div className="flex justify-center items-center py-24">
        <div className="animate-pulse flex flex-col items-center space-y-4">
          ...
          <span className="text-[10px] tracking-[0.3em] text-neutral-400 uppercase font-sans">Loading Reviews</span>
        </div>
      </div>
    );
  }
        ...
        {pressItems.map((item, index) => {
          ...
          return (
            <motion.div
              ...
              className="group relative bg-neutral-950/40 hover:bg-neutral-950 border border-neutral-900 hover:border-neutral-800 p-8 md:p-10 rounded-sm flex flex-col justify-between min-h-[280px] transition-all duration-500 overflow-hidden"
            >
              {/* Corner accent line */}
              ...
              <div className="space-y-6">
                    ...
                    {Array.from({ length: 5 }).map((_, i) => (
                      <Star 
                        ...
                        className={`w-3.5 h-3.5 ${
                          i < rating 
                            ? 'text-neutral-300 fill-neutral-300/20' 
                            : 'text-neutral-800'
                        }`} 
                      />
                    ))}
                ...
                {/* Big Quote */}
                <p className="font-serif text-lg md:text-xl lg:text-2xl font-light text-neutral-200 group-hover:text-white leading-relaxed tracking-wide transition-colors duration-300">
                  “{translatedQuote}”
                </p>
              </div>

              {/* Source & Read Action button */}
              <div className="border-t border-neutral-950/60 pt-6 mt-8 flex justify-between items-end">
                <div className="space-y-1">
                  <h4 className="font-serif text-sm tracking-widest text-white uppercase font-normal">
                    {item.source}
                  </h4>
                  {item.author && (
                ...
                {item.link && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center space-x-1.5 text-[10px] tracking-widest text-neutral-400 group-hover:text-white uppercase transition-all duration-300 transform translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100"
                  >
