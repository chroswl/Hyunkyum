        ...
        {categories.map((cat) => (
          <button
            ...
            className={`px-5 py-2 text-xs tracking-[0.2em] uppercase transition-all border rounded-full duration-300 whitespace-nowrap ${
              activeCategory === cat
                ? 'bg-white/10 text-white border-white font-semibold shadow-sm'
                : 'bg-transparent text-neutral-400 border-neutral-900 hover:text-white hover:border-neutral-700 font-normal'
            }`}
          >
        ...
        <AnimatePresence mode="popLayout">
          {filteredItems.map((item, index) => (
            <motion.div
              ...
              className="break-inside-avoid relative group overflow-hidden cursor-pointer bg-neutral-950 rounded-sm border border-neutral-900/60"
              ...
              <div className="relative overflow-hidden w-full h-full">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6">
                  <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                    <span className="text-[10px] tracking-widest text-neutral-300 uppercase font-sans font-semibold">
                      {item.category === 'Portrait' ? t.portraitCat : item.category === 'Stage' ? t.stageCat : item.category === 'Backstage' ? t.backstageCat : ''}
                    </span>
                    <h4 className="text-sm font-serif font-light text-white tracking-wide mt-1">
                      {getTranslatedTitle(item)}
                    </h4>
                    <div className="mt-3 flex items-center space-x-1.5 text-[10px] text-neutral-400">
        ...
        {selectedItemIndex !== null && (
          ...
          >
            ...
            <button
              id="lightbox-close"
              onClick={() => setSelectedItemIndex(null)}
              className="absolute top-6 right-6 text-neutral-400 hover:text-white transition-colors p-2 bg-neutral-900/40 rounded-full border border-neutral-800"
            ...
            {filteredItems.length > 1 && (
              <>
                <button
                  id="lightbox-prev"
                  onClick={handlePrev}
                  className="absolute left-4 md:left-8 text-neutral-400 hover:text-white transition-colors p-3 bg-neutral-900/40 hover:bg-neutral-900/60 border border-neutral-800 rounded-full"
                ...
                <button
                  id="lightbox-next"
                  onClick={handleNext}
                  className="absolute right-4 md:right-8 text-neutral-400 hover:text-white transition-colors p-3 bg-neutral-900/40 hover:bg-neutral-900/60 border border-neutral-800 rounded-full"
            ...
            >
              ...
              <div className="mt-4 text-center max-w-2xl px-4">
                <span className="text-[10px] tracking-[0.2em] text-neutral-400 uppercase font-semibold">
                ...
                <h3 className="text-lg md:text-xl font-serif font-light text-white tracking-wide mt-1">
